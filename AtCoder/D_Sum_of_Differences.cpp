...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        int val = brr[i];
        int k = upper_bound(arr.begin(), arr.end(), val) - arr.begin();
        
        int bcont = (k*(val%tmod))%tmod;
        bcont = (bcont - pref[k] + tmod)%tmod;

        int sumg = (pref[n]- pref[k] +tmod)%tmod;
        int term2 = (sumg -((n-k)*(val%tmod))%tmod + tmod)%tmod;

        ans = (ans + bcont + term2)%tmod;
    }
