...
void solve() {
    ...
    for(int i = 0 ; i  < m ; i++) {
        ...
        pair<int,int> tl = pts[i];
        pair<int,int> tr = make_pair(tl.first+1,tl.second);
        pair<int,int> br = make_pair(tr.first,tr.second -1);
        pair<int,int> bl = make_pair(br.first - 1,br.second);

        if((occ.find(tl) != occ.end()) || (occ.find(tr) != occ.end()) || (occ.find(bl) != occ.end()) || (occ.find(br) != occ.end())) {
            // do nothing
        } else {
            // cout<<" inserting : "<<tl.first<<" "<<tl.second<<"\n";
            occ.insert(tl);
            occ.insert(tr);
            occ.insert(bl);
            occ.insert(br);
